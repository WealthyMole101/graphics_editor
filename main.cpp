...
private:
    ...
    //\property
    std::shared_ptr<graphic_view> _view;
    //\property
    std::shared_ptr<document> _doc;
    bool _is_end_process = false;

public:
    explicit controller(std::shared_ptr<graphic_view> view): _view(view)
    {
...
//            graphic_primitive *gp = figures.begin()->get();
            ...
            _doc.get()->delete_figure(*figures.begin());
        }
    }
...
int main()
{
    std::shared_ptr<graphic_view> view;
